...
[[account_contracts_contactclassconfigrepositorycontract]]
...
Possible values: any property of ContactClass model
...
a|`$filters`
|link:http://php.net/array[array^]
a|Optional: Filter results by different criteria. Possible values: id, name
|===
...
====== *Return type:*        xref:Account.adoc#account_models_contactclass[`ContactClass`]
...
public batchUpdate($contactsBatch):bool
...
a|`$contactId`
|link:http://php.net/int[int^]
a|The ID of the contact. Returns `true` if deletion was successful.
Returns `false` if deletion was not successful.

a|`$checkExistingOrders`
|link:http://php.net/bool[bool^]
a|Flag that checks if the contact is linked to orders.
If the contact is linked to orders, CustomerDeleteException is thrown and the contact will not be deleted.
|===
...
public searchContacts($filters, $with, $page, $itemsPerPage, $sortBy, $sortOrder):Plenty\Repositories\Models\PaginatedResult
...
|Name |Type |Description
a|`$filters`
|        xref:Account.adoc#account_complex_contactsearchrequestfilters[`ContactSearchRequestFilters`]
a|
...
|position_id
    |link:http://php.net/int[int^]
    a|The job position ID of the contact
|===
...
[[account_models_contactbatchupdateentry]]
...
|showPrice
    |link:http://php.net/int[int^]
    a|Flag to display price in the online store as Gross(1) or Net(2)
|discountType
    |link:http://php.net/int[int^]
    a|
|position
...
|allowedMops
    |link:http://php.net/string[string^]
    a|The list of allowed payment methods
|pdwQuantity1
    |link:http://php.net/float[float^]
    a|
|pdwDays1
...
public search($page = 1, $itemsPerPage = 50, $filters = [], $sortBy = &quot;contactId&quot;, $sortOrder = &quot;desc&quot;, $with = &quot;&quot;, $returnAsArray = true):Plenty\Repositories\Models\FilteredPaginatedResult
...
public searchAfter($itemsPerPage = 50, $filters = [], $sortBy = &quot;id&quot;, $sortOrder = &quot;desc&quot;, $with = &quot;&quot;, $searchAfterKey = [], $withResult = true, $returnAsArray = true):array
...
[[account_filter_complex]]
===  Complex
[[account_complex_contactsearchrequestfilters]]
==== ContactSearchRequestFilters
...
`Plenty\Modules\Account\Contact\Search7\Filter\Complex`
...
a|`$with`
|link:http://php.net/array[array^]
a|Optional: The relationships that should be eager loaded. (example ['contacts']).
|===
...
|externalId
    |link:http://php.net/string[string^]
    a|The externalId for this company (like the hubspot company id)
|createdAt
    |
...
[[account_job_contracts]]
===  Contracts
[[account_contracts_jobrepositorycontract]]
...
====== *Return type:*        xref:Account.adoc#account_models_job[`Job`]
...
a|`$jobId`
|link:http://php.net/int[int^]
a|The ID of the job. Returns `true` if deletion was successful.
Returns `false` if deletion was not successful.
|===
...
a|`$jobData`
|link:http://php.net/array[array^]
a|The job data as associative array
...
a|`$page`
|link:http://php.net/int[int^]
a|The page of results to search for [optional, default = 1]

a|`$itemsPerPage`
|link:http://php.net/int[int^]
a|The number of items to list per page [optional, default = 50]
...
a|`$filters`
|link:http://php.net/array[array^]
a|Filter that restrict the search result. Filter can be for name or jobId
...
|id
    |link:http://php.net/int[int^]
    a|The ID of the job name
|positionId
    |link:http://php.net/int[int^]
    a|The job ID
|lang
    |link:http://php.net/string[string^]
    a|The language of the job name
|name
    |link:http://php.net/string[string^]
...
[source%nowrap, php]
[#batchdeleterecipientsbyid]
----

public batchDeleteRecipientsById($recipientIds):int
...
a|`$recipientIds`
|link:http://php.net/array[array^]
a|An array of recipient IDs to delete
|===
...
public searchFolders($page, $itemsPerPage, $sortBy, $sortOrder, $filters):Plenty\Repositories\Models\PaginatedResult
...
a|`$page`
|link:http://php.net/int[int^]
a|Page number for pagination

a|`$itemsPerPage`
|link:http://php.net/int[int^]
a|Number of items per page for pagination
...
a|`$sortOrder`
|link:http://php.net/string[string^]
a|The order to sort by (asc or desc)

a|`$filters`
|link:http://php.net/array[array^]
a|Filters to apply to the search
|===
...
public listRecipients($columns, $page, $itemsPerPage, $filter, $with, $append = [], $sortBy = null, $sortOrder = null):Plenty\Repositories\Models\PaginatedResult
...
a|`$append`
|link:http://php.net/array[array^]
a|An array with custom attributes to be appended
...
public moveRecipients($sourceFolderId, $destinationFolderId):int
...
a|`$sourceFolderId`
|link:http://php.net/int[int^]
a|The ID of the source folder from which the recipients will be moved

a|`$destinationFolderId`
|link:http://php.net/int[int^]
a|The ID of the destination folder to which the recipients will be moved
|===
...
public moveRecipientsById($recipientIds, $destinationFolderId):int
...
a|`$recipientIds`
|link:http://php.net/array[array^]
a|An array of recipient IDs to be moved
...
public searchOrders($page, $itemsPerPage, $sortBy, $sortOrder, $filters):Plenty\Repositories\Models\PaginatedResult
...
a|`$page`
|link:http://php.net/int[int^]
a|The page number for pagination
...
public importFromOrders($folderId, $filters):int
...
a|`$folderId`
|link:http://php.net/int[int^]
a|The ID of the folder to which the email addresses will be imported
...
|isDeletable
    |link:http://php.net/bool[bool^]
    a|Typo, wrong property name, should be isDeleteable.
|isDeleteable
    |link:http://php.net/bool[bool^]
    a|Flag that indicates if the newsletter folder can be deleted.
The folders 'Customers' and 'Interested parties' are available by default and cannot be deleted.
|position
    |link:http://php.net/int[int^]
    a|The position of the newsletter folder
|isSelectable
    |link:http://php.net/bool[bool^]
    a|Flag that indicates if the newsletter folder can be selected by customers
in the online store. If this is allowed, the folder will be displayed in the My account area of the online store.
Customers will then be able to subscribe to the newsletters that are included in this folder.
|numberOfRecipients
    |link:http://php.net/int[int^]
    a|The number of recipients in the newsletter folder
|===
